...
func getProjectRoot() string {
	...
	if err == nil {
		...
		// If running from tmp/ (Air hot-reload), go up one level
		if filepath.Base(dir) == "tmp" {
			dir = filepath.Dir(dir)
		}
		// Verify it's the project root by checking for a known directory
...
func main() {
	...
	// Resolve project root and change working directory so all relative paths work
	rootDir := getProjectRoot()
	if err := os.Chdir(rootDir); err != nil {
		log.Fatalf("Gagal mengubah working directory ke %s: %v", rootDir, err)
	}
	...
	// Auto-disposal: recalculate missing retention dates, then check for
	// archives past retention and create pemusnahan entries automatically.
	// Runs at startup and then periodically every 24 hours.
	// Skip on Vercel (serverless functions don't support long-running goroutines).
	if !isVercel {
	...
	// Use gin.New() instead of gin.Default() to suppress HTTP request logs.
	// Only Recovery middleware is added; Logger is omitted to keep terminal clean.
	r := gin.New()
	r.Use(gin.Recovery())
	...
	// request to the /database-setup page so the connection can be repaired
	// from the browser instead of crashing. Static assets above are exempt.
	r.Use(func(c *gin.Context) {
		if database.DB == nil {
...
auth.GET("/arsip", arsipH.Index)
	...
	// Internal authorization (admin only while connected) is enforced in the
	// handlers themselves.
